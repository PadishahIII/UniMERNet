...
class MathDataset(Dataset):
    def __init__(self, config_name="plain", transform=None):
...
def load_data(image_path, math_file):
    ...
    if len(image_paths) != len(math_gts):
        raise ValueError(
            f"The number of images does not match the number of formulas. {len(image_paths)} != {len(math_gts)}"
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--result_path", type=str, help="Path to json file to save result to."
    )
    parser.add_argument(
        "--options",
        nargs="+",
    ...
    parser.add_argument("--config-name", default="plain", help="'plain' or 'styled'")
...
def main():
    ...
    print(f"Evaluation Set:{args.config_name}-validation")
